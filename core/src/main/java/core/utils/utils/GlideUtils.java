...
import core.widget.CircleImageView;
import core.widget.CircleTransform;
...
public abstract class GlideUtils {
    ...
    private static <T, K> DrawableTypeRequest<K> getContext(T context, K url) {
        ...
        try {
            ...
            e.printStackTrace();
            Logger.e("GlideUtils-DrawableTypeRequest", "Glide的Context错误");
        }
     ...
     */
    public static void loadImageFormVideo(Context context, String url, ImageView imageView) {
        Glide.with(imageView.getContext()).load(url)
                .override(DensityUtils.dp2px(context, 150), DensityUtils.dp2px(context, 150))
        ...
        public boolean onResourceReady(K resource, T model, Target<K> target, boolean isFromMemoryCache, boolean isFirstResource) {
            if (null != imageLoadListener) {
                if (imageView != null) {
                    ...
                    Logger.i("GlideUtils-onResourceReady", "加载完成回调");
